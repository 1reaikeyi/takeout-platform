<template>
  <div class="setmeal-container">
    ...
    >
      ...
      <el-table-column prop="categoryName" label="套餐分类" min-width="100" />
      <el-table-column label="套餐价" min-width="100">
        <template #default="{ row }">
    ...
    <el-empty v-if="!loading && tableData.length === 0" description="暂无套餐数据">
      <el-button type="warning" @click="handleAdd">添加第一个套餐</el-button>
    </el-empty>
    ...
    <el-pagination
      ...
      :total="total"
      @size-change="fetchSetmealList"
      @current-change="fetchSetmealList"
    />
  </div>
</template>
...
import { getSetmealList, deleteSetmeal, updateSetmealStatus } from '@/api/setmeal'
...
const handleStatusChange = async (row) => {
  ...
  try {
    await ElMessageBox.confirm(`确认${actionText}该套餐?`, '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    })

    await updateSetmealStatus(row.id, newStatus)
    ElMessage.success(`套餐${actionText}成功`)
    fetchSetmealList()
  } catch (error) {
    if (error !== 'cancel') {
...
const handleBatchDelete = async () => {
  ...
  try {
    await ElMessageBox.confirm('确定删除选中的套餐?', '确定删除', {
      confirmButtonText: '删除',
      cancelButtonText: '取消',
      type: 'warning'
    })
