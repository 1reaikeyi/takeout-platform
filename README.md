...
一个基于 Spring Boot 3 + Vue 3 的全栈外卖餐饮管理系统，提供完整的用户端和管理端功能，支持菜品管理、套餐管理、订单处理、支付集成、实时通信等核心能力。
...
数据库初始化脚本位于 [database/sql.txt](database/sql.txt)，包含所有业务表的建表语句：

- **员工表** (employee) — 管理员与员工账号
- **用户表** (user) — 消费者用户
- **分类表** (category) — 菜品/套餐分类
- **菜品表** (dish) — 菜品信息与口味配置
- **套餐表** (setmeal) — 套餐信息
- **套餐-菜品关联表** (setmeal_dish) — 套餐与菜品关联
- **购物车表** (shopping_cart) — 用户购物车
- **订单表** (orders) — 订单主表
- **订单明细表** (order_detail) — 订单子项
- **地址簿表** (address_book) — 收货地址
